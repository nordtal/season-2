 ...
 *
 * The path is {@code ["phase", "show"]} on both surfaces, because Discord cannot invoke a command
 * that has subcommands. The proxy additionally runs it for a bare {@code /phase}, which is a
 * Brigadier tree detail rather than a second command.
 */
public final class PhaseCommands {
     ...
     *
     * Every admin command in the repository is console and web only, and a single set of four that
     * stayed reachable from chat would be the exception nobody remembers - {@code /phase launch} is
     * the most consequential command in the project, since it starts the season.
     *
     * {@link Surface#WEB} rather than {@link Surface#CONSOLE} alone: the Season page in Steward is
     * where a person already reads the phase, so it is where they should be able to set it. A WEB
     * row is pinned to the asker's Discord id by a CHECK, so "who launched the season" stays
     * answerable - which a CONSOLE row, pinned to having no identity at all, cannot answer for the
     * one command where it matters most. Anybody holding that console can edit
     * {@code season_phase} directly anyway; that is an argument about what a console <em>can</em>
     * do, not about where this command should be offered.
     */
     ...
     *
     * <b>Not</b> irreversible: setting the date again is an exact undo. It only drives the MOTD
     * countdown and the pre-opening disconnect screens, so a wrong value is visible at once and
     * costs one more command to fix.
     */
     ...
     *
     * Irreversible, unlike {@link #LAUNCH}: moving it shifts every access period that has not
     * started yet, and moving it back shifts them again against the clock rather than undoing the
     * first shift - a grant that started in between is no longer moved at all.
     */
