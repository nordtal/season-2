...
public final class ShowAccess implements NordtalCommand<SmpEffects> {
    ...
    public void run(final NordtalUser user, final Values values, final SmpEffects effects) {
        ...
        effects.async(() -> {
            ...
            final Optional<SmpEffects.Access> access = readAccess(user, effects, player, name);
            if (access.isEmpty()) {
                return;
            }
    ...
    private static Optional<SmpEffects.Access> readAccess(
            final NordtalUser user, final SmpEffects effects, final UUID player, final String name) {
