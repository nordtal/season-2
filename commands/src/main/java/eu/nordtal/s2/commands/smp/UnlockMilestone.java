 ...
 *
 * The blunt version of {@link CompleteObjective}, and the one that cannot be undone in any useful
 ...
 * that was reached early.
 *
 * Unlike completing an objective, an unknown key is not checked first. The engine is the only
 * thing that knows the whole track - including milestones that are not active - and asking it twice
 * would be two reads for a command an admin runs a handful of times a season.
 */
