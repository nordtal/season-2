...
final class JdbiAccessRequests implements AccessRequests {
    ...
    public AccessRequest submit(final NewAccessRequest request, final Duration patience) {
        ...
        return dao.submit(request.kind().name(), request.subject(), request.argument(),
                request.source().name(), request.requestedBy(), seconds);
    }
    ...
    public void finish(final long id, final boolean ok, final String result) {
        final int written = dao.finish(id, ok ? AccessRequestStatus.DONE.name()
                : AccessRequestStatus.FAILED.name(), result);
        if (written == 0) {
            ...
            System.getLogger(JdbiAccessRequests.class.getName())
                    .log(System.Logger.Level.WARNING,
                            "access request " + id + " was not RUNNING when it was settled");
        }
    }
