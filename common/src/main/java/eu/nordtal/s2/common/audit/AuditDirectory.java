 ...
 *
 * <h2>What the table is for, and why reading it is its own API</h2>
...
public interface AuditDirectory {
     ...
     *
     * <p>One statement whichever way it is called - the predicates switch themselves off rather than
     ...
     *
     * <p>Both filters are exact matches, not substring searches. {@code action} is drawn from a small
     ...
     */
    List<AuditEntry> search(String action, String subject, int limit);

    /**
     * Writes one line into the journal.
     *
     * <p>Whoever performs an action records it - there is no central recorder watching the tables,
     ...
     * read to answer.</p>
     *
     * <p>It is deliberately a separate statement from the action it describes rather than part of
     * its transaction. A grant that lands without its journal line is a bug worth finding; a member
     * locked out because the bookkeeping failed is worse than the bug.</p>
     *
     ...
     */
    void record(String action, String actor, String subject, java.util.UUID mcUuid, String detail);
}
