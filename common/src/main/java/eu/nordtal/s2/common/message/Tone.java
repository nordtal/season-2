 ...
 *
 * <b>Why a shared command may say this, when it may not say a colour</b>
 *
 ...
 *
 * So the <em>meaning</em> travels and the rendering does not. A command names a tone; each
 * adapter decides what that is on its surface. Paper and Velocity colour the component through
 * {@link Tones}; Discord ignores it, because an embed has one colour for the whole of it.
 *
 * <b>Why it is here and not in {@code :commands}</b>
 *
 ...
 *
 * It is a bare enum with no method on it on purpose. {@link Tones} is separate because it names
 * Adventure, and the Discord bot loads this type - through {@code NordtalUser}'s signature - in a
 * JVM that has no Adventure on its classpath at all.
 */
public enum Tone {

    /** Nothing to flag, an ordinary reply; painted grey so it looks the same everywhere. */
    NEUTRAL,
     ...
     *
     * The artefact versions under a service, and a service that is not moving at all. Present so
     * that a report reads as a few lines with detail under them rather than as forty equal ones.
     */
    MUTED
}
