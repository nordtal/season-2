 ...
 * arguments - a Component argument has to appear as its {@code <tag>} instead.
 *
...
public final class MessageSpecCheck {
    ...
    public static List<String> problems(final Class<?> spec) {
        ...
        for (final MessageSchema.Entry entry : entries) {
            ...
            final List<String> plain = MessageSchema.argsByKind(entry).get(false);
            final List<String> components = MessageSchema.argsByKind(entry).get(true);
            if (new HashSet<>(plain).size() + new HashSet<>(components).size() != entry.args().size()) {
            ...
            check(problems, key, "en", english.getProperty(key), plain, components);
            if (german.containsKey(key)) {
                check(problems, key, "de", german.getProperty(key), plain, components);
            }
        }
    ...
    private static void check(final List<String> problems, final String key, final String language,
                              final String text, final List<String> plain, final List<String> components) {
