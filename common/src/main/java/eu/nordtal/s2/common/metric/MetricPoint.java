 ...
 *
 * Not the same shape as {@link MetricSample} on purpose, and the difference is the two fields
 * that move.
 *
 * The subject and the metric are gone: {@link MetricDirectory#range} was asked for one series,
 * so repeating its name on every one of up to 86 400 points is a string per point that the caller
 * already knows.
 *
 * {@link #resolution()} appears instead, because a range that crosses the compaction boundary
