...
public final class PackAdvances {
    ...
    public static Map<Integer, Integer> of(final String fontFile) {
        ...
        final JsonObject root =
                JsonParser.parseString(RepositoryRoot.read(fontFile)).getAsJsonObject();
        for (final JsonElement element : root.getAsJsonArray("providers")) {
                ...
                for (final Map.Entry<String, JsonElement> advance :
                    ...
                    advance.getKey()
                            ...
                            .forEach(codePoint -> table.putIfAbsent(
                                    codePoint, advance.getValue().getAsInt()));
                }
                continue;
            }

            final BufferedImage image =
                    read(FontFile.texturePath(provider.get("file").getAsString()));
            final int rows = provider.getAsJsonArray("chars").size();
            final int columns = provider.getAsJsonArray("chars")
                    .get(0)
                    .getAsString()
                    .codePointCount(
                            0,
            ...
            for (int row = 0; row < rows; row++) {
                final int[] codePoints = provider.getAsJsonArray("chars")
                        .get(row)
                        .getAsString()
                        .codePoints()
                        .toArray();
                for (int column = 0; column < codePoints.length; column++) {
                    ...
                    final int rightmost = rightmost(image, column * cellWidth, row * cellHeight, cellWidth, cellHeight);
