 ...
 *
 * {@code :common} 's access API asks by Minecraft account, which a Discord interaction does not have, so the same
 * question is asked by {@code discord_id} here.
 *
 * Read-only on purpose: {@link GuildState} writes the flag, mirrored from the Discord admin role. An admin is
 * appointed in Discord and nowhere else.
 */
interface AdminFlagDao {
     ...
     *
     * Two queries where one would do, deliberately: they are asked a handful of times a season by an admin
     * waiting on a deferred interaction.
     *
     * @param discordId the account to ask about
     ...
     *
     * An account with no row is not an admin. {@link #isAdmin(String)} deliberately does not do it itself, so
