...
| `paper-common` | library | What the three Paper plugins do identically **with a Paper type** — `:common` is compiled against no platform, so this is where a Bukkit-facing helper lives instead of in three copies | **new 2026-09-04** — the operator adapter, and the home for the Brigadier adapters `:commands` will need |
