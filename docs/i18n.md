...
replaced by a list. A deployed `access.yml` still carrying one of them loses that line on the next
start, with a warning and a `.bak` — this paragraph said "deleting a key is only free while nothing
is deployed, because an undeclared key stops the config load outright" until jcore 3.1.0 on
2026-09-05, which was true then and is the reason the sentence is corrected rather than removed:
...
paper-common/src/main/resources/messages/paper-common/{en,de}.properties
```

All six exist. `limbo`'s were written on 2026-09-01 and `smp`'s with the world half the same day -
...
**What every Paper server says about a player has its own bundle, since 2026-09-09.**
`paper-common/src/main/resources/messages/paper-common/{en,de}.properties` holds the five system
lines - chat, join, leave, death, advancement - because they are the same five sentences on `smp` and
on `hunger-games` and were one server's until that day. A Paper plugin therefore loads **three**
roots, most general first: `paper-common`, then `commands`, then its own. Later roots win, so a
server can reword a shared line without editing the shared file.
