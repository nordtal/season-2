...
smp/src/main/resources/messages/smp/{en,de}.properties              # not yet — nothing in smp renders to a player yet
```

The first four exist; `limbo`'s were written on 2026-09-01. `smp`'s bundle waits on the world half:
the server-free core built the same day has no player-facing string in it, and inventing an empty
bundle for it would only make the next session delete one. **`limbo`'s bundle is unusual and worth
