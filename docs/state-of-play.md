...
| 19 | **New 2026-09-01, third pass:** Gradle does not track `.env.example` as an input of `:discord-bot:test`, although `ConfigsTest` reads it | **stands, and its premise was wrong — re-checked 2026-09-01, seventh pass.** `ConfigsTest` does **not** read that file: it holds a hand-copied literal of the language list and the price list. So the Gradle input is moot and the real problem is worse than the one written here — two copies of the same block with nothing comparing them, which is exactly the failure mode this repository refuses everywhere else. The fix is for the test to read the file and for the convention plugin to declare it an input; until then, an edit to `.env.example` is unchecked by anything |
