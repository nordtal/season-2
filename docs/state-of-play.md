...
after the fact. **Findings 9, 19, 22 and 30 still stand**; every other row is closed. Keeping that sentence
...
| **A second login in one client session leaves the waiting room.** Seen on 2026-09-05 with a real client on the production proxy: the first join of the day was released to `smp` after ~20 s; the second and third (after a proper disconnect) sat on `limbo` under "Loading the resource pack" for more than six minutes, with no `apply-timeout` kick either. The proxy therefore booked no `SUCCESSFUL` and ran no timeout — which points at either a `WaitingBook` session outliving the disconnect (so `claimOffer` sends no second offer) or the sweep not running. The proxy log for that window is in [`todo.md`](../../todo.md) as the owner's step | the login-path rehearsal, from the proxy log | Forget the session on `DisconnectEvent` if it is the first; schedule the sweep if it is the second. Until then an admin leaves with `/server smp` and everybody else stares at a black screen with a title, which is the exact failure the timeout exists to turn into a message |
