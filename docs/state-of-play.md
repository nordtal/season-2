...
| 22 | **New 2026-09-01, fourth pass:** nothing publishes the Minecraft image. `.env.example` suggested `#MC_IMAGE=ghcr.io/nordtal/minecraft:0.1.0` and `release.yml` pushes the bot image and no other | **stands, documented.** compose builds it on the host from `./minecraft` and tags it `local`, which works — but it means the host needs this repository checked out and a Docker able to build, not merely a place to keep `.env`. The comment says so now |
| 23 | **New 2026-09-01, fifth pass (building the updater):** the fix for finding 21 cannot survive the module it was written for. `compose.yml` sets `NORDTAL_NETWORK_CONTROL_PACK_URL` and `..._SHA1`, and **a jcore environment override wins over the file and is never written back to it** — so an updater writing a new sha1 into `pack.yml` would write into a value nothing reads: a swap that reports success and changes nothing | **decided, not yet done.** Both variables leave `compose.yml` and `.env.example` again in step 3 of [updater.md](updater.md#two-things-this-displaced), and `pack.yml` becomes the place. Until then the environment is authoritative and the updater's report reads the file, so the two can disagree — which is why the pack row names the full forty-character hash rather than a prefix |
