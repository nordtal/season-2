...
appended. **Findings 9, 19 and 22 still stand**; every other row is closed. Keeping that sentence
...
| **Arcane's redeploy endpoint** — the path is not in the public documentation and the token is on the production host, so the restart button is the one part of the updater nobody here can finish | the first deployment; [`todo.md`](../../todo.md) §7 | Leave `arcane.base-url` empty. Everything else in the updater works, both surfaces say "Arcane is not configured", and the redeploy is a click in Arcane. **A 2xx from a real Arcane has never been seen**; the 404, 401, 503 and unreachable branches are all tested, and a 404 was driven end to end through the container |
