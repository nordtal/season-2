...
| `commands` | 46 files, 4131 lines | 178 | **Every admin command in the network**, its declaration, its decisions, and both ends of the request row |
...
none of `smp`'s 176 tests touches a world, a packet or a player, because none of them can, and this
...
after the fact. **Findings 9 and 30 still stand**; every other row is closed (19 and 22 by their own rows, 33 on 2026-09-02 when the packages went public, 54 by the command table in `docs/README.md`). Keeping that sentence
...
| 54 | **New 2026-09-04, and open.** **Eight commands exist in no document.** `/smp farmreset`, `/smp objective complete`, `/smp milestone unlock`, `/smp aura`, `/poi add`, `/poi remove`, `/hg ready` and `/hg ready-status` are implemented and reachable, and `grep` finds none of them in `docs/`. `docs/smp.md` describes the three escape hatches as a design feature over two paragraphs and **never names the command that operates any of them**, so an admin reading the documentation cannot use the feature it argues for | **open.** The five reload commands added by finding 44 are in the same state and are the same edit |
...
|---|---|---|
| **A Minecraft client follows GitHub's redirect** when downloading the pack. The redirect itself is measured ([resource-pack/README.md](../resource-pack/README.md#hosting)); whether a *client* follows it is not | the login-path rehearsal | Host the zip on a small static HTTP host. The URL and hash are already configuration, so it is a config edit plus one more thing to keep alive and certificated on event day — cheap, but nobody should discover it that morning. **Half of that fallback now exists**: `compose.yml`'s `devpack` profile is exactly such a host, built on 2026-09-05 so the local stack can serve a pack it just built. It is loopback-only and development-only, but it is the same shape, so the fallback is a config edit and a profile rather than something to write that morning |
...
| **Operator actually covers what an admin needs**, including nodes belonging to third-party plugins nobody enumerated. Replaced `smp`'s six-node `PermissionAttachment` on 2026-09-04, and no test here can say anything about it: `AdminOperatorsTest` proves the *policy*, not that Paper's operator flag opens the doors we think it does | the SMP rehearsal, with a real client on all three servers | Go back to a node list, but a long one and on all three servers rather than only `smp` — which is strictly better than what was there before this change, because `hunger-games` and `limbo` attached nothing at all. The retired `admin-permissions` key is the shape to restore; `ConfigsTest` names it |
| **`ops.json` is empty after a restart on every Paper server.** The enable-time sweep is what makes an operator a property of the session rather than of the disk, and it is the half of this change that a crash can defeat | the SMP rehearsal | The sweep runs too early or not at all. Falling back means moving it later in `onEnable` or into the first tick — it is one call, and until it works a crash leaves an operator on disk that nothing removes |
