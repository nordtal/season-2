...
| 50 | **New 2026-09-04, and open.** `docs/smp.md` §"the milestone track" says a milestone unlocking is a **server-wide event — "title and chat announcement in every player's language, announced in Discord as well"**. `ObjectiveEngine#announceMilestone` sends **a chat line**. There is no title and there is no Discord announcement. The German string even says *"Schau nach oben"* — look up — at a sky where nothing happens | **open.** The balloon half of that sentence does hold: `BalloonGui` reads `season.unlocked()` live, so the greyed entry does light up |
| 51 | **New 2026-09-04, and open.** `docs/smp.md`'s farm-reset sequence says the warnings go out at 30 / 10 / 5 / 1 minutes **"chat + HUD"**. `FarmWorldReset#warn` sends chat only; `SmpHud` has a status bar and a navigation bar and no third line | **open.** One of the two has to move — and the HUD is the half that reaches somebody who is not reading chat while mining |
...
| 53 | **New 2026-09-04, and open.** **There is no sound and no particle anywhere in this repository.** `playSound`, `Sound.`, `spawnParticle` and `Particle.` return zero hits across all seven modules. A season with a wheel of fortune, a hot-air balloon, duels, graves, milestone completions, a shrinking border and a battle-royale countdown is entirely silent. `showTitle` appears in exactly one game surface, `limbo`'s waiting room — the least interactive server on the network is the only one that uses the screen | **open, and it is the largest single gap in how the season feels.** Nothing in the docs promises a specific sound, which is why nothing caught it: it is absent from the code *and* from the plan |
...
| 58 | **New 2026-09-04, and open.** **`nordtal:board` is fully drawn and used by nothing.** Twenty-two textures, twenty-eight code points, corners, edges, dividers and its own negative-advance space provider — and `Boards.java` writes plain text onto a Text Display without touching one `BOARD_*` constant. The geometry is specified (`resource-pack/README.md`: cells connect at 4.5/4.5, so any width butts up seamlessly) | **open, and decided:** the frame gets built. Two questions belong to the owner and are in `todo.md` — the board's width, and whether the interior divider is drawn |
