...
|---|---|---|---|
| `smp` | 94 files, 13274 lines | 176 | Everything docs/smp.md describes, none of it yet seen on a running server |
...
| `commands` | 46 files, 4131 lines | 171 | **Every admin command in the network**, its declaration, its decisions, and both ends of the request row |
...
1197 tests, none skipped, all green with a Docker daemon present (`./gradlew build`, 2026-09-05,
...
after the fact. **Findings 9, 30 and 52 still stand**; every other row is closed (19 and 22 by their own rows, 33 on 2026-09-02 when the packages went public, 54 by the command table in `docs/README.md`). Keeping that sentence
...
| 52 | **New 2026-09-04, and open.** `docs/smp.md` §"Discord" says *"The bot gets a narrow SMP surface — announcements, logs, and commands for admins and users."* **The bot has no SMP surface at all**: no announcement, no log, no command. Its eight slash commands are access, phase, update and messages | **open, and it is the counterpart of finding 50** — the Discord half of a milestone announcement is the same missing wire |
