package eu.nordtal.s2.networkcontrol.feedback;
...
/**
 * The one place in {@code network-control} that names a sound to Velocity.
 *
 ...
 */
public final class NetworkControlSounds implements CommandGate.Chime {

    private final FeedbackSounds sounds;
    private final Consumer<String> problems;

    NetworkControlSounds(final FeedbackSounds sounds, final Consumer<String> problems) {
        this.sounds = sounds;
     ...
     *                 {@code logger::warn}
     */
    public static NetworkControlSounds defaults(final Consumer<String> problems) {
        ...
        return new NetworkControlSounds(FeedbackSounds.parse(declared, problems), problems);
    }
