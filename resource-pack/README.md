...
`nordtal/font/gui_r0.json` … `gui_r5.json` and `:common`'s `Glyphs` are all *mirrors* of the tables
...
- **Put the `github.com/<owner>/<repo>/releases/download/<tag>/<file>` URL in the config, never the
  ...
  **`release-assets.githubusercontent.com`**, carrying a signature that expires in well under an
  ...
  tonight. Whether a Minecraft *client* follows that redirect is still unverified; a static host is
...
**Status column.** *keep* and *keep — season 1* rows exist today, in the pack and in `Glyphs`.
*generated — final candidate* is programmatically drawn geometry plausible as shipping art;
*generated — placeholder* is a stand-in that exists only so the font, the HUD and the board can be
exercised on a running server before the real design happens. *removed* rows are gone from the pack,
...
The six *positive* advances `\uFFF01`–`\uFFF32` exist because **the width of a rendered line is the
one quantity nothing here can compute** — a board's content is text in `minecraft:default`, whose
per-character advances live in the client. So the frame never places anything *after* the content: a
...
The divider has the same power-of-two tiling as `edge_h`, because a rule *inside* the board must
...
`fblue`, `fgreen`, `fred` and `fwhite` are **land-status flags** — the `f` is *flag*, not a boss bar
...
A menu *title* needs none of these — the panel is drawn from the left edge and the readable text
...
places, so the only thing that varies per player is a card's *state*. Each state is one tile-sized
...
forced: the player's rows sit one pixel *higher* than the texture has them, because `ChestScreen`
...
*take out of*, and there the separate cells say these are distinct stacks and any one may be taken.
...
separately that no *other* two characters draw the same picture, with a named list of the pairs that
