 ...
 *
 * The roster was never the problem - network-control writes it every ten seconds and the worker puts
 * it on the `network-control` row as `roster`. The interface simply never read it: `useOnline` took
...
describe("useOnline - the roster comes from the same row the total does (steward/111)", () => {
  it("hands on the people network-control wrote down", () => {
    services([
      {
  ...
  it("keeps an empty list when the row carries no roster at all - absence, not a guess", () => {
    services([{ service: "network-control", players: 4 }])
